 ...
 *
 * Single command:
 *   GO<in>         e.g. GO24           drive forward N inches
 *   BACK<in>       e.g. BACK12         drive backward N inches
 ...
 *   RECAL                              recalibrate IMU gyro offsets
 *
 * Sequence (comma-separated, no spaces):
 *   GO60,LEFT90,GO60,RIGHT90,GO80,TURN180
 *
 * Responses:
 ...
 *   RECAL_START          beginning gyro recalibration
 *   RECAL_DONE           recalibration complete
 *
 * Board:     Arduino Nano
...
const float WHEEL_DIAMETER_M = 0.080;   // meters — adjust if robot over/undershoots

const int DRIVE_PWM = 95;
...
// BR encoder faulty — not attached
...
void recalibrateIMU() {
  stopRobot();
  Serial.println(F("RECAL_START"));
  Serial.println(F("Calibrating gyro — keep robot still..."));
  delay(2000);
  mpu.calcGyroOffsets();
  lastIMUTime = millis();
  Serial.println(F("RECAL_DONE"));
}
...
// Shared core for goStraight and goBack — positive drivePWM = forward, negative = backward
// targetInches is always positive; direction is controlled by drivePWM sign.
// Encoders always count rising edges regardless of direction, so we use abs counts.

void goStraight(float targetInches) {
  float targetM      = targetInches * 0.0254;
  float targetPulses = (targetM / (PI * WHEEL_DIAMETER_M)) * PULSES_PER_REV;
  unsigned long lastProgress = 0;
  float lastPrintedInches = 0;

  noInterrupts();
  encFL = 0; encFR = 0; encBL = 0;
  interrupts();
  ...
  while (true) {
    ...
    noInterrupts();
    long snapFL = abs(encFL), snapFR = abs(encFR), snapBL = abs(encBL);
    interrupts();

    float avgPulses = (snapFL + snapFR + snapBL) / 3.0;

    float traveledM = (avgPulses / PULSES_PER_REV) * (PI * WHEEL_DIAMETER_M);
    float traveledInches = traveledM / 0.0254;

    float remaining = targetInches - traveledInches;
...
// print every 0.1 inch change
if (abs(traveledInches - lastPrintedInches) >= 0.1) {
    lastPrintedInches = traveledInches;

    Serial.print(F("PROGRESS "));
    Serial.println(traveledInches);
}
    ...
    if (now - lastHeadingUpdateTime >= HEADING_INTERVAL_MS) {
      ...
      int leftPWM  = constrain(DRIVE_PWM - correction, -255, 255);
      int rightPWM = constrain(DRIVE_PWM + correction, -255, 255);
...
void goBack(float targetInches) {
  float targetM      = targetInches * 0.0254;
  float targetPulses = (targetM / (PI * WHEEL_DIAMETER_M)) * PULSES_PER_REV;
  unsigned long lastProgress = 0;
  float lastPrintedInches = 0;
  ...
  setSides(reversePWM, reversePWM);
...
if (abs(traveledInches - lastPrintedInches) >= 0.1) {
    ...
    if (now - lastHeadingUpdateTime >= HEADING_INTERVAL_MS) {
      ...
      float correction =
        (headingKp * 1.3 * headingError) +
        (headingKi * headingIntegral) +
        (headingKd * (-gyroZ));

      correction = constrain(correction, -60, 60);

      // reverse-specific correction
      int leftPWM  = constrain(reversePWM - correction, -255, 255);
      int rightPWM = constrain(reversePWM + correction, -255, 255);
...
bool readSerial() {
  while (Serial.available()) {
    ...
    if (c == '\n' || c == '\r') {
      cmdBuffer.trim();
      if (cmdBuffer.length() > 0) return true;
    } else {
      cmdBuffer += c;
    }
  }
...
bool executeSingle(const String& cmd) {
  ...
  if (parseParam(cmd, "GO", param)) {
    if (param <= 0) { Serial.println(F("ERR:GO_PARAM")); return false; }
    goStraight(param);                   // param is inches

  } else if (parseParam(cmd, "BACK", param)) {
    if (param <= 0) { Serial.println(F("ERR:BACK_PARAM")); return false; }
    goBack(param);                       // param is inches
