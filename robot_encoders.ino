...
// ── Motor trim ────────────────────────────────────────────────────────────────
// Compensates for hardware motor imbalance before the PID gets involved.
// Positive value = boost right side / reduce left side (corrects drift right).
// Negative value = boost left side / reduce right side (corrects drift left).
// Start at 5 and increase in steps of 5 until the robot drives roughly straight
// before PID correction, then fine tune with headingKp.
const int TRIM = 5;
...
unsigned long lastHeadingPIDTime = 0;
...
// Internal version used by heading correction — trim already applied
void _setSidesRaw(int leftPWM, int rightPWM) {
  setFL(leftPWM);  setBL(leftPWM);
  setFR(rightPWM); setBR(rightPWM);
}
...
void loop() {
  ...
  if (now - lastIMUTime >= IMU_INTERVAL_MS) {
    float dt_imu = (now - lastIMUTime) / 1000.0;
    lastIMUTime  = now;
    mpu.update();
    gyroZ        = mpu.getGyroZ();
    totalAngleZ += gyroZ * dt_imu;
  }
  ...
  // ── IMU heading PID correction ────────────────────────────────────────────
  // Suppressed during ramp — engages only once at full speed
  if (drivingStraight) {
    unsigned long nowH = millis();
    float dt_h         = (nowH - lastHeadingPIDTime) / 1000.0;
    lastHeadingPIDTime = nowH;
    ...
    float headingDerivative = -gyroZ;  // gyroZ is already the rate of change
    ...
    int leftPWM  = constrain((DRIVE_PWM - TRIM) + (int)headingCorrection, 0, 255);
    int rightPWM = constrain((DRIVE_PWM + TRIM) - (int)headingCorrection, 0, 255);
    _setSidesRaw(leftPWM, rightPWM);
  }
